...
class BioProject():
	...
	def verifyData(self):
		...
		if self.gse == 'GSE77388':
			self.experimentName = self.experimentName + "_2"
	...
	def getUniquePubMedId(self, PubMedIds):
		if len(PubMedIds) > 0:
			...
			return ','.join(self.alternative_pubmedid)
	...
	def getInfo(self, gse):
		...
		Sample          = self.serachItem(Samples, 'Item', 'Name', 'Sample')
	...
	def getSRAOfSRP(self, DocSum):
		...
		Title = data['root']['Library_descriptor']['LIBRARY_NAME']
		if Title == None:
			Title = data['root']['Experiment']['@name']
		...
		return {
			...
			#'library_name': data['root']['Library_descriptor']['LIBRARY_NAME']
			'taxid': data['root']['Organism']['@taxid']
		}
	...
	def getSRAs(self, Samples, GSEweb):
		SRAs = []
		if not str(type(Samples['Item'])) == "<class 'list'>":
		...
		for sample in Samples['Item']:
			...
			####
			### for gse: GSE25124 - if Accession not in ['GSM617220','GSM617214','GSM617200','GSM617190','GSM617218','GSM617216']:
			### for gse: GSE34585 - if Accession not in ['GSM851624','GSM851627','GSM851625','GSM851628','GSM851626']:
			if Accession not in ['GSM573521']:
	...
	def runARun(self, DocSum):
		...
		if condition['status'] == 'ok':
			...
			condition_value = SRA + '_' + condition_value.replace(' ', '_').replace(':', '_').replace(',', '_').replace('=', '_').replace('(', '').replace(')', '')
	...
	def getLibraryOfSRA(self, Accession):
		...
		XML          = self.getValue(ExpXml)
		data         = xmltodict.parse('<root>' + XML + '</root>')
		response     = {'library': '', 'taxid': ''}
		if self.existsKey(data['root'], 'Library_descriptor'):
				if self.existsKey(data['root']['Library_descriptor'], 'LIBRARY_STRATEGY'):
					response['library'] = data['root']['Library_descriptor']['LIBRARY_STRATEGY']

		if self.existsKey(data['root'], 'Organism'):
				if self.existsKey(data['root']['Organism'], '@taxid'):
					response['taxid'] = data['root']['Organism']['@taxid']
	...
	def serachCondition(self, SRAS, value, cond):
		for sra in SRAS:
			if sra[cond] == value:
				if (sra['library'] == 'RNA-Seq' or sra['library'] == 'EST') and (sra['taxid'] == '3055' or sra['taxid'] == '3077'):
